...
void* CModule::FindVirtualTable(const std::string& name)
{
	...
	{
		printf("Failed to find .rodata or .data.rel.ro section\n");
		return nullptr;
	}
	...
	if (!classNameString)
	{
		printf("Failed to find type descriptor for %s\n", name.c_str());
		return nullptr;
	}
	...
	if (!typeName)
	{
		printf("Failed to find type name for %s\n", name.c_str());
		return nullptr;
	}
	...
	printf("Failed to find vtable for %s\n", name.c_str());
	return nullptr;
}
#endif
